...
export function toSnapshot(fs) {
    return {
        ...
        fsStats: Object.fromEntries([...fs._state.fsStats].map(([path, fsStat]) => {
            return [
                path,
                {
                    ...fsStat,
...
export function fromSnapshot(fs, snapshot, { pathPrefix = "" } = {}) {
    ...
    fs._state.fsStats = new Map(Object.entries(snapshot.fsStats).map(([path, rawStat]) => {
...
export function createNodeishMemoryFs() {
    // local state
    const state = {
        lastIno: 1,
        fsMap: new Map(),
     ...
     * 2 = Symlink
     */
    function newStatEntry(path, stats, kind, modeBits, target) {
        ...
        const mtimeMs = Math.floor(currentTime / 1000) === (oldStats?.mtimeMs && Math.floor(oldStats?.mtimeMs / 1000))
            ? currentTime + 1000
            : currentTime;
