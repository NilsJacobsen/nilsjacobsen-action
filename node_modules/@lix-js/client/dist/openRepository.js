...
const { clone, listRemotes, status, statusMatrix, push, pull, currentBranch, add, log, listServerRefs, checkout, addRemote, fetch: gitFetch, commit: isoCommit, findRoot, merge, } = isoGit;
...
export async function openRepository(url, args) {
    ...
    const enableExperimentalFeatures = whitelistedExperimentalRepos.includes(`${owner}/${repoName}`.toLocaleLowerCase());
    ...
    function delayedAction({ execute }) {
        if (pending) {
    ...
    return {
         ...
         */
        async getOrigin({ safeHashOnly = false } = {}) {
            if (safeHashOnly) {
